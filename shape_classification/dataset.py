...
class ProjectOnSphere:
    ...
    def __init__(self, dataset, image_shape, normalize=True):
        ...
        self.sgrid = convert_spherical_to_3d(torch.stack((lon, lat), -1)).view(
            -1, 3).double().numpy()
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(level={0}, points={1})'.format(
            self.level, self.pts)
...
class ModelNet(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.bispherical_resample_to_texture = ResampleToUVTexture(
            image_shape, base_order, sample_order, 1, 'bispherical')
    ...
    def __getitem__(self, index):
        ...
        img = torch.from_numpy(img.reshape(1, -1, *self.image_shape))
        img = self.bispherical_resample_to_texture(img).permute(
            0, 2, 1, 3, 4).squeeze(0).contiguous().numpy()
